...
export default function Contact({ cols, services }) {
    ...
    // if cols type is object convert to array
    const contactFields = typeof cols == "object" ? Object.values(cols) : cols;
    const serviceLabels =
        typeof services == "object" ? Object.values(services) : services;
    ...
    // store checkbox values seperately into session and pass into values
    function handleCheckbox(e) {
        const key = e.target.id;
        //updating check key and values in session.
        let checks = JSON.parse(sessionStorage.getItem("checks"));
        if (e.target.checked) {
            const value = e.target.value;
            if (checks !== null) {
                checks[key] = value; //updating if exists
                sessionStorage.setItem("checks", JSON.stringify(checks));
            } else {
                checks = { [key]: value }; //setting up new object
                sessionStorage.setItem("checks", JSON.stringify(checks));
            }
        }
        // remove data if unchecked
        // https://stackoverflow.com/questions/33053310/remove-value-from-object-without-mutation#answer-58249788
        else {
            const { [key]: remove, ...rest } = checks; //removes the unchecked key value and return other remaining key values in "rest"
            checks = rest;
            sessionStorage.setItem("checks", JSON.stringify(checks));
        }

        const serviceValues = Object.values(checks); //Get array values
        setValues((values) => ({
            ...values,
            ["service"]: JSON.stringify(serviceValues),
        }));
    }
                    ...
                    <div className=" flex flex-wrap lg:justify-between">
                                ...
                                <div class="mb-8 flex w-full max-w-[370px]">
                                    ...
                                    <div class="w-full">
                                        ...
                                        <p class="text-body-color text-base">
                                            {info.company_address ? info.company_address : 'Not set'}
                                        </p>
                                    </div>
                                </div>
                                <div class="mb-8 flex w-full max-w-[370px]">
                                    ...
                                    <div class="w-full">
                                        ...
                                        <p class="text-body-color text-base">
                                        {info.telephone_num ? `${info.telephone_num}` : info.phone_num ? `${info.phone_num}` : 'Not set'}
                                        </p>
                                    </div>
                                </div>
                                <div class="mb-8 flex w-full max-w-[370px]">
                        ...
                        <div className="w-full lg:w-1/2 xl:w-6/12">
                            <form
                                ...
                                method="post"
                                className="relative rounded-lg bg-secondary py-5 shadow-lg px-5"
                            >
                                <div>
                                                    ...
                                                    <div className="flex flex-row items-center flex-wrap">
                                                        {serviceLabels.map(
                                                            (sLabel, index) => (
                                                                ...
                                                                >
                                                                    <Checkbox
                                                                        label={sLabel.title.toUpperCase()}
                                                                        id={
                                                                            sLabel.title
                                                                        }
                                                                        value={
                                                                        ...
                                                                        // onChange={()=>{handleChange(sLabel.title)}}
                                                                        // passing change function to the child component
                                                                        handleChange={
                                                                            handleCheckbox
                                                                        }
                                                                        name={
